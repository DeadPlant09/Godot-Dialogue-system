...
@export var starting_conversation_id = 1
...
func _ready() -> void:
	collision_layer = 0
	if starting_conversation_id >= 1 and ending_conversation <= 0: ending_conversation = starting_conversation_id
...
func save_dialogue():
	starting_conversation_id = Diauloge_Systerm.conversation_id # set the current conversation_id to the Diauloge_Systerm current conversation_id
	ending_conversation = Diauloge_Systerm.Ending_Conversation
	
	if Debug:
		print(str(get_parent().name) + "saving_dialouge")


func load_dialogue():
	if not ending_conversation < 0: # the conversation can't end with 0
		Diauloge_Systerm.Ending_Conversation = ending_conversation
	if not starting_conversation_id < 1 and not starting_conversation_id > ending_conversation:
		# And set the Diauloge_Systerm.conversation_id to our conversation_id
		Diauloge_Systerm.conversation_id = starting_conversation_id
